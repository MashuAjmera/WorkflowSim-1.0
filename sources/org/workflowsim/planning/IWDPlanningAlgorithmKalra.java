 ...
 * @date Oct 12, 2013
 */
public class IWDPlanningAlgorithmKalra extends BasePlanningAlgorithm {

	public class setBestPriorityArray {
...
//	MAIN
	public static double av = 1, bv = 0.01, cv = 1, as = 1, bs = 0.01, cs = 1, tau = 1, fizero = 0.5, lr = 0.9,
			gr = 0.1, delsoilmin = 10, delsoilmax = 90, es = 0.01, ev = 0.0001, a = 2, globalsoilupdate = 0.1;
	public static Boolean bestPathFlag = false;
	public static int[] bestPathPriorityArray;
	public static double MakeSpan = 50;
	public static double bestSoil = 0.0;

	public static void setBestPathFlag(Boolean var) {
		bestPathFlag = var;
	}

	public static void setBestPriorityArray(int[] best_priority) {
		bestPathPriorityArray = best_priority;
	}

	public static void setMakeSpan(double var) {
		MakeSpan = var;
	}
	...
	private class Graph {
		...
		void updateGlobaledge(int i, int j) {
			double wt = (1 + globalsoilupdate) * getWeight(i, j) - globalsoilupdate * bestSoil * MakeSpan;
			modifyEdge(i, j, wt);
...
//		double updateBestPathSoil(int i, int j, double VEL) {
//			double w = this.getWeight(i, j);
//			w = 1 + gr * w - gr * soilIWD * (1 / bestMakeSpan);
//			this.modifyEdge(i, j, w);
//		}
...
//		public void setglobaledgeweights() {
//			for (int i = 0; i < V; i++) {
//				for (int j = 0; j < adj[i].size(); j++) {
//					adj[i].get(j).weight = (1 + gr) * adj[i].get(j).weight - gr * soilIWD * bestMakeSpan;
//				}
//			}
//		}
...
//				g.removeEdge(currNode);
				...
				x /= 100;
//				System.out.println("Here random number " + x);

				for (int j = 0; j < m.size(); j++) {
//					System.out.println("Probarr [j] " + probArrN[j]);

					if (probArrN[j] >= x) {
						nextNode = m.get(j);

//						System.out.println("currNode " + currNode + " nextNode " + nextNode + " j " + j + "Probarr "
//								+ probArrN[j]);
