...
import org.workflowsim.planning.IWDPlanningAlgorithmKalra;
...
public class IWDAlgorithmKalraExample {
	protected static List<CondorVM> createVM(int userId, int vms) {
		...
		list.add(vm[1]);
		vm[2] = new CondorVM(2, userId, mips * 3, 1, 7500, bw, 840000, vmm, 0.175, costPerMem, costPerStorage,
				costPerBw, new CloudletSchedulerSpaceShared());
		list.add(vm[2]);
		vm[3] = new CondorVM(3, userId, mips * 4, 1, 15000, bw, 840000 * 2, vmm, 0.35, costPerMem, costPerStorage,
				costPerBw, new CloudletSchedulerSpaceShared());
		list.add(vm[3]);
		vm[4] = new CondorVM(3, userId, mips * 5, 1, 15000, bw, 840000 * 2, vmm, 0.35, costPerMem, costPerStorage,
				costPerBw, new CloudletSchedulerSpaceShared());
		list.add(vm[4]);
		...
		for (int i = 0; i < 5; i++) {
			sum += list.get(i).getCurrentRequestedTotalMips();
		}
		System.out.println("RequestedMips" + sum / 200000 + "%");
		System.out.println("RequestedMips" + list.get(1).getTotalUtilizationOfCpu(CloudSim.clock()));
	...
	public static void main(String[] args) {
		...
		try {
			double bestMakeSpan = 1000000000000.0;
			int[] best_priority = Cloudlet.getArray();
			...
			for (int i = 0; i < 50; i++) {
				 ...
				 * planner
				 */
				Parameters.SchedulingAlgorithm sch_method = Parameters.SchedulingAlgorithm.IWDK;
				Parameters.PlanningAlgorithm pln_method = Parameters.PlanningAlgorithm.IWDK;
				...
				double compareMakeSpan = makespan(outputList0);
				if (compareMakeSpan < bestMakeSpan) {
					bestMakeSpan = compareMakeSpan;

					IWDPlanningAlgorithmKalra.setMakeSpan(printJobList(outputList0)[0]);
					IWDPlanningAlgorithmKalra.setBestPathFlag(true);
					best_priority = Cloudlet.getArray();
				} else {
					IWDPlanningAlgorithmKalra.setBestPathFlag(false);
				}
//				minMakeSpan = Math.min(minMakeSpan, printJobList(outputList0));

				CloudSim.stopSimulation();
			}
			IWDPlanningAlgorithmKalra.setBestPriorityArray(best_priority);
			System.out.println("MinMakeSpan " + bestMakeSpan + " MinCost " + bestCost);
//			System.out.println("Makespan" + printJobList(outputList0));
	 ...
	 */
	protected static double makespan(List<Job> list) {
		double makespan = 0.0;
		for (Job job : list) {
	...
	protected static double[] printJobList(List<Job> list) {
		String indent = "    ";
		Log.printLine();
